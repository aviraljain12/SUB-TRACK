...
function generateInvoice(paymentId, userId, res) {
    db.get(
        "SELECT p.*, u.full_name, u.email_address FROM payments p JOIN users u ON p.user_id = u.user_id WHERE p.payment_id = ?",
        [paymentId],
        async (err, payment) => {
            if (err || !payment) {
...
app.post("/api/signup", async (req, res) => {
    ...
    try {
        ...
        db.get("SELECT * FROM users WHERE email_address = ?", [email], async (err, user) => {
            if (err) {
            ...
            try {
                ...
                // Insert new user
                db.run(
                    "INSERT INTO users (full_name, email_address, password_hash) VALUES (?, ?, ?)",
...
app.post("/api/subscribe", verifyToken, (req, res) => {
    ...
    db.run(
        `INSERT INTO subscriptions (user_id, service_name, plan_name, billing_amount, subscription_status, start_date, next_renewal_date)
         VALUES (?, ?, ?, ?, 'active', ?, ?)`,
        ...
        function (err) {
            ...
            db.run(
                `INSERT INTO payments (user_id, service_name, plan_name, paid_amount, payment_date)
                 VALUES (?, ?, ?, ?, ?)`,
...
app.get("/api/subscription/:service", verifyToken, (req, res) => {
    ...
    db.get(
        `SELECT * FROM subscriptions WHERE user_id = ? AND service_name = ?`,
        [user_id, service],
        (err, subscription) => {
            if (err) {
...
app.post("/api/subscription/:service/cancel", verifyToken, (req, res) => {
    ...
    db.run(
        `UPDATE subscriptions 
         ...
         WHERE user_id = ? AND service_name = ? AND subscription_status = 'active'`,
        [user_id, service],
        function(err) {
            if (err) {
...
app.get("/api/download-invoice/:id", verifyToken, (req, res) => {
    ...
    try {
        ...
        db.get(
            `SELECT p.*, u.full_name, u.email_address 
             FROM payments p 
