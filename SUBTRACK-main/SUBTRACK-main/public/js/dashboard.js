        ...
        if (response.ok) {
            ...
            invoices.forEach(invoice => {
                ...
                invoiceItem.innerHTML = `
                    ...
                    <button class="download-btn" onclick="downloadInvoice(${invoice.payment_id})">
...
function downloadInvoice(invoiceId) {
    const token = localStorage.getItem('token');
    if (!token) return;
    window.open(`${window.API_BASE_URL}/api/download-invoice/${invoiceId}?token=${token}`, '_blank');
}
...
function displaySubscriptions(subscriptions) {
    ...
    subscriptions.forEach(subscription => {
        const service = window.SERVICES_CONFIG[subscription.service_name];
        if (service) {
            const renewalInfo = getRenewalStatus(subscription.next_renewal_date);
            const card = document.createElement('div');
            card.className = `subscription-card ${renewalInfo.class}`;
            
            // Calculate next billing amount including tax
            const baseAmount = parseFloat(subscription.billing_amount);
