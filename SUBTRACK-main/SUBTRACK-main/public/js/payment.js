document.addEventListener('DOMContentLoaded', () => {
    ...
    payButton.addEventListener('click', async () => {
        ...
        try {
            ...
            existingSubscriptions = existingSubscriptions.filter(sub => sub.service_name !== service);
            existingSubscriptions.push(subscriptionData);
            localStorage.setItem('subscriptions', JSON.stringify(existingSubscriptions));

            // Redirect to success page
            window.location.href = `payment-success.html?invoice=${responseData.invoice_id}&service=${service}&plan=${plan}&amount=${amount}`;
        } catch (error) {
