            ...
            subscriptionDetails.innerHTML = `
                <div class="subscription-section">
                    ...
                    <div class="subscription-info">
                        ...
                        <div class="info-row">
                            <label>Price:</label>
                            <span>₹${subscription.billing_amount}</span>
                        </div>
                        <div class="info-row">
                            <label>Status:</label>
                            <span class="status ${subscription.subscription_status.toLowerCase()}">${subscription.subscription_status}</span>
                        </div>
                        <div class="info-row">
                ...
                <div class="subscription-section">
                    ...
                    <div class="plans-grid">
                        ${Object.entries(serviceInfo.plans)
                            .map(([planId, plan]) => `
                                <div class="plan-card ${planId === subscription.plan_name ? 'current' : ''}">
                                    <h4>${plan.name}</h4>
...
async function loadBillingHistory(service, token) {
    try {
        ...
        const serviceInvoices = invoices.filter(invoice => invoice.service_name === service);
                ...
                ${serviceInvoices.map(invoice => `
                    <div class="invoice-item">
                        <div class="invoice-details">
                            <span class="date">${new Date(invoice.payment_date).toLocaleDateString()}</span>
                            <span class="plan">${invoice.plan_name}</span>
                            <span class="amount">₹${invoice.paid_amount}</span>
                        </div>
                        <a href="${window.API_BASE_URL}/api/download-invoice/${invoice.payment_id}?token=${token}" class="download-btn" target="_blank">
